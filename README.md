...
- [Updating to NewComponent Releases](#updating-to-new-releases)
...
## Updating to NewComponent Releases
...
In the WebStorm menu `Run` select `EditComponent Configurations...`. Then click `+` and select `JavaScript Debug`. Paste `http://localhost:3000` into the URL field and save the configuration.
...
page (showing a "NewComponent content is available; please refresh." message). Showing
...
When users install your app to the homescreen of their device the default configuration will make a shortcut to `/index.html`. This may not work for client-side routers which expect the app to be served from `/`. EditComponent the web app manifest at [`public/manifest.json`](public/manifest.json) and change `start_url` to match the required URL scheme, for example:
